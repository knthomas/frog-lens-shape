...
which(!species_stage_means$genus_species %in% as.vector(tree_orig$tip.label))
...
Here, we prune the phylogeny to the 123 species in our dataset, and then relabel the tip labels to match our names for the species. The phylogeny we are using has two species that differ in name from the species in our dataset (synonyms), and one species that is not represented. Specifically, in the phylogeny we replace _Hypsiboas geographicus_ with synonym _Boana geographica_, _Syncope hudsoni_ with synonym _Chiasmocleis hudsoni_, and _Hylarana temporalis_ with synonym _Indosylvirana temporalis_. We replace _Rhombophryne_alluaudi_ with substitute _Rhombophryne_nilevina_ and _Elachistocleis ovalis_ with substitute _Elachistocleis nigrorgularis_.
