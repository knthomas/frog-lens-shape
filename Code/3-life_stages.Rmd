...
```{r procD all metrics, eval = FALSE}

fit <- summary(
  procD.lm(species_stage_means[,c("mean_anisotropy","mean_elongation","mean_sphericity")]~life_stage,iter = 999,seed = NULL,RRPP = TRUE, SS.type = NULL, int.first = FALSE, Cov = NULL, data = species_stage_means, print.progress = TRUE)
  ) 
...
#fit alternative model with lambda set to 1
aniso.pgls2 <- pgls(mean_anisotropy.diff~env, data=geocaperdata, lambda=1)
...
#fit model with lambda set to 1
sphere.pgls2 <- pgls(mean_sphericity.diff~env, data=geocaperdata, lambda=1)
...
#view coefficients
summary(sphere.pgls2)

#fit model with lambda set to 0
sphere.pgls3 <- pgls(mean_sphericity.diff~env, data=geocaperdata, lambda=0.00001)
...
#fit model again with lambda set to 1
elong.pgls2 <- pgls(mean_elongation.diff~env,data=geocaperdata, lambda=1)
...
matchData$lab <- ifelse(matchData$eco=="aquatic", "aq. to aq.", "aq. to terr.")
matchData$lab <- factor(matchData$lab, levels = c("aq. to aq.", "aq. to terr."))
...
an.paired.plot2 <- ggplot(matchData, aes(x = stage, y = anisotropy)) +
  ...
  geom_segment(aes(x=1, y = 0.37, xend = 2, yend = 0.37, col=hab))
